...
def main():
    ...
    trainloader = data.DataLoader(
        CIFAR10RandomLabels(root='/home/xh1007/marginbound/cifar10/data', train=True, download=True,
                        num_classes=num_classes, transform = transform_train,
                            corrupt_prob=0.0),
        batch_size=train_batch, shuffle=True, num_workers=workers)
    testloader = data.DataLoader(
        CIFAR10RandomLabels(root='/home/xh1007/marginbound/cifar10/data', train=False,
                   num_classes=num_classes, transform = transform_test,
                            corrupt_prob=0.0),
    ...
    for epoch in range(start_epoch, epochs):
        ...
        sys.stdout.flush()
        if ((epoch+1)%20 == 0):
            torch.save(model.state_dict(), 'model_'+str(epoch+1)+'_6')            
        # # save model
